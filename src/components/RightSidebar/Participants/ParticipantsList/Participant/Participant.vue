...
<template>
	<li class="participant-row"
		...
		<Actions v-if="canBeModerated && !isSearched"
			...
			<ActionButton v-if="canResendCallNotification"
				:close-after-click="true"
				@click="resendCallNotification">
				<template #icon>
					<Bell :size="20"
						title=""
						decorative />
				</template>
				{{ t('spreed', 'Resend call notification') }}
			</ActionButton>
...
export default {
	...
	methods: {
		...
		async resendCallNotification() {
			try {
				await this.$store.dispatch('resendCallNotification', {
					token: this.token,
					attendeeId: this.attendeeId,
				})
